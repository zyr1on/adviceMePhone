...
class OptimizedPhoneDataset(Dataset):
    ...
    def __getitem__(self, idx):
        input_text = f"query: {str(self.inputs[idx])}"
...
class OptimizedPhoneT5Model:
    ...
    def train_progressive(self, data_file, output_dir='./optimized_phone_t5', total_epochs=50):
        ...
        # 3 aşamalı eğitim
        stages = [
            {"epochs": 15, "lr": 5e-4, "batch_size": 32, "stage": 0}, # batch_size değiştirilebilir aslında
            {"epochs": 20, "lr": 3e-4, "batch_size": 16, "stage": 1}, 
            {"epochs": 15, "lr": 1e-4, "batch_size": 16, "stage": 2}
        ]
    ...
    def generate_response(self, input_text, max_length=256, num_beams=5, temperature=0.7):
        ...
        input_ids = self.tokenizer.encode(
            formatted_input,
            return_tensors='pt',
            max_length=128, # 256 da yapılabilr
            truncation=True
        )
