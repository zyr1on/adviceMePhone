...
class PhonePredictor:
    ...
    def predict(self, input_text):
        ...
        for feature_text in features:
            processed_text = self.advanced_preprocess_text(feature_text)
        ...
        return predictions_list, confidences_list, raw_outputs
    
    def recommend_phones(self, predictions_list, confidences_list, top_n=None):
        """Tahminlere göre telefon öner"""
        filtered_df = self.phones_df.copy()
        applied_filters = []
        
        # Her bir özelliğin tahminlerini sırayla filtrele
        for idx, (predictions, confidences) in enumerate(zip(predictions_list, confidences_list)):
    ...
    def format_output(self, predictions_list, confidences_list, raw_outputs, recommendations):
        """Sonuçları güzel formatta göster"""
        for idx, (raw_output, predictions, confidences) in enumerate(zip(raw_outputs, predictions_list, confidences_list)):
            print("\n" + "="*60)
        ...
        # Önerileri göster
        self.recommend_phones(predictions_list, confidences_list)
    ...
    def interactive_mode(self):
        ...
        print("Örnek: 'kamerası iyi olsun, ios olsun, 8gb ram olsun'")
        ...
        while True:
            try:
                ...
                predictions_list, confidences_list, raw_outputs = self.predict(user_input)
                self.format_output(predictions_list, confidences_list, raw_outputs, None)
                
                self.save_prediction_log(user_input, predictions_list, confidences_list)
    ...
    def save_prediction_log(self, input_text, predictions_list, confidences_list):
...
def main():
    ...
    try:
        ...
        if len(sys.argv) > 3:
            ...
            predictions_list, confidences_list, raw_outputs = predictor.predict(input_text)
            predictor.format_output(predictions_list, confidences_list, raw_outputs, None)
        else:
